  ...
  <section class="payroll-page">
    <header class="payroll-header panel-dark">
      <div>
        ...
        <p v-if="canReadSalary">{{ period.employeeCount }} 人 · {{ periodStatusLabel }}</p>
      ...
      <div class="header-actions">
        ...
        <button v-if="canReviewSalary || canLockSalary" class="dark-button" type="button" @click="reviewDrawerOpen = true">复审中心</button>
        <button v-if="canWriteSalary" class="dark-button" type="button" @click="openProfileDrawer">薪资标准设置</button>
        <button class="dark-button" type="button" @click="printPreview">打印预览</button>
        <button v-if="canReviewSalary" class="light-button" type="button" :disabled="!canSubmitReview" @click="submitReview">提交主管复审</button>
        <button v-if="canLockSalary" class="muted-button" type="button" :disabled="period.status !== 'second_review'" @click="lockPeriod">锁定工资周期</button>
        <button v-if="canCreateRevision" class="light-button" type="button" :disabled="saving" @click="createRevision">创建 v{{ period.versionNo + 1 }} 修订版</button>
      </div>
    </header>
      ...
      <main class="ledger-panel panel">
        ...
        <div class="ledger-scroll" :class="{ 'is-empty': visibleEmployees.length === 0 }">
          <div class="ledger ledger-head">
            <span>员工 / 工种</span><span>薪资标准</span><span class="col-r">工时/月</span><span class="col-r">加项</span><span class="col-r">扣项</span><span class="col-r">实发工资</span><span>记录</span><span>初审状态</span><span class="col-r">操作</span>
          </div>
          <button v-for="employee in visibleEmployees" :key="employee.id" class="ledger ledger-row" :class="{ selected: selectedEmployee?.id === employee.id, blocked: employee.review_status === 'blocked' }" type="button" @click="selectEmployee(employee)">
            ...
            <span class="group-cell col-r"><b class="mono positive">+{{ money(additions(employee)) }}</b><small class="parts">全勤{{ brief(employee.attendance_award) }}·绩效{{ brief(employee.performance_award) }}·补贴{{ brief(employee.allowance) }}</small></span>
            <span class="group-cell col-r"><b class="mono negative">−{{ money(deductions(employee)) }}</b><small class="parts">扣款{{ brief(employee.deduction) }}·五险{{ brief(employee.social_insurance_housing_fund) }}</small></span>
            ...
            <span><em class="status-mark" :class="employee.review_status">{{ reviewStatusLabel(employee.review_status) }}</em></span><span class="row-action col-r">查看 · {{ employee.review_status === 'locked' ? '打印' : '初审' }}</span>
          </button>
        ...
        <section v-if="selectedEmployee" class="employee-detail">
          ...
          <div class="change-history"><span class="label">修改记录</span><button v-if="selectedEmployee.is_manually_modified" class="history-row" type="button" @click="toggleHistory"><b>{{ historyLoading ? '读取中…' : '查看字段级修改凭证' }}</b><span>操作人、原因、前后值、附件 →</span></button><p v-else class="history-empty">当前工资快照没有人工修改记录。</p><div v-if="showHistory" class="history-expanded"><p v-if="historyError">{{ historyError }}</p><p v-else-if="history.length === 0">暂无变更记录。</p><div v-for="record in history" v-else :key="record.id" class="history-record">{{ record.changed_field }}：{{ record.before_value || '空' }} → {{ record.after_value || '空' }} · {{ record.changed_by_name || '未知操作人' }} · {{ record.created_at }}</div></div></div>
          <button v-if="canReviewSalary && period.status === 'first_review' && selectedEmployee.review_status === 'pending'" class="outline-button" type="button" :disabled="saving" @click="reviewSelected">完成本人工资初审</button>
          <button v-if="canWriteSalary && period.status === 'first_review'" class="primary-button" type="button" @click="openEditDrawer">编辑本人工资快照</button>
        </section>
      </main>
    ...
    <div v-if="reviewDrawerOpen" class="drawer-backdrop" @click.self="reviewDrawerOpen = false">
      <aside class="review-panel panel review-drawer">
        ...
        <span class="label">复审说明</span><textarea v-model.trim="reviewNote" :disabled="!canReviewSalary || period.status !== 'first_review'" placeholder="说明复审依据、异常处理或凭证编号" />
        <div class="lock-note"><b>锁定规则</b><p>锁定后 salary 快照只读；错误需创建同月 v2，v1 永久保留。</p></div>
        <button v-if="canReviewSalary" class="muted-wide" type="button" :disabled="!canSubmitReview" @click="submitReview">完成阻断项后提交复审</button><button v-if="canLockSalary" class="muted-wide" type="button" :disabled="period.status !== 'second_review'" @click="lockPeriod">锁定工资周期</button><button v-if="canCreateRevision" class="muted-wide" type="button" :disabled="saving" @click="createRevision">创建 v{{ period.versionNo + 1 }} 修订版</button>
      </aside>
    </div>
...
export default defineComponent({
  ...
  setup() {
    ...
    const canSubmitReview = computed(() => canReviewSalary.value && period.status === "first_review" && period.unconfiguredCount === 0 && period.reviewedCount === period.employeeCount && period.employeeCount > 0);
    ...
    const selectEmployee = (row: SalaryEmployeeRow) => { selectedEmployee.value = row; showHistory.value = false; history.value = []; historyError.value = ""; };
    ...
    const loadEmployees = async (targetPage = currentPage.value) => {
      ...
      if (!rows) {
        ...
        if (result.period) {
          period.status = result.period.status; period.versionNo = result.period.versionNo;
    ...
    const reviewSelected = async () => { if (!selectedEmployee.value) return; saving.value = true; try { await financeApi.reviewPayrollEmployee(selectedEmployee.value.id); await refreshEmployees(); } finally { saving.value = false; } };
    const submitReview = async () => { saving.value = true; try { await financeApi.submitPayrollReview(reviewNote.value); await refreshEmployees(); } finally { saving.value = false; } };
    const lockPeriod = async () => { saving.value = true; try { await financeApi.lockPayroll(); await refreshEmployees(); } finally { saving.value = false; } };
    const createRevision = async () => { saving.value = true; try { await financeApi.createPayrollRevision(); await refreshEmployees(); } finally { saving.value = false; } };
...
.ledger{min-width:0;grid-template-columns:minmax(0,1.5fr) minmax(0,1fr) minmax(0,.7fr) minmax(0,1.15fr) minmax(0,1.15fr) minmax(0,1fr) minmax(0,.85fr) minmax(0,.85fr) minmax(0,.8fr)}
