...
public:
    explicit doctorHandler(std::shared_ptr<DatabaseManagerInterface> db) : BaseHandler(db) {}

    crow::response getDoctor(const crow::request& req);             // 获取上班医生信息  对应 /api/doctor/getDoctor

    crow::response doctorHandler::getUserList(const std::string &name);   // 获取用户信息 对应 /api/doctor/getUserList
