...
crow::response ReservationHandler::createReservation(const crow::request &req, int user_id, std::string name, std::string email, std::string phone, int doctor_id, std::string date, std::string time_slot, std::string status)
{
    try
    {
        ...
        {
            // 创建数据库插入操作
            mysqlx::TableInsert insert_op = reservation_table.insert("user_id", "doctor_id", "date", "time_slot", "status");

            // 插入数据库
            try
            {
                insert_op.values(user_id, doctor_id, date, time_slot, status).execute();
            }
...
crow::response ReservationHandler::getReservations(const crow::request &req, int user_id)
{
    try
    {
        ...
        try
        {
            ...
            // 查询该用户的所有预约记录
            mysqlx::RowResult result = reservation_table.select("id", "user_id", "doctor_id", "date", "time_slot", "status", "created_at")
    ...
    try
    {
        ...
        try
        {
            ...
            mysqlx::RowResult result = dbManager->getSession()
                                           ->sql("SELECT od.doctor_id, u.name, "
                                                 "u.phone, u.email "
                                                 "FROM onlineDoctors AS od "
                                                 "JOIN users AS u ON od.doctor_id = u.id "
                                                 "WHERE od.date = ? AND od.status = 'online'")
                                           .bind(todayDate)
                                           .execute();
