...
namespace DatabaseMigrations::ForeignKeys
{
    namespace
    {
        // 检查并为指定表添加缺失的外键，避免重复执行 ALTER TABLE。
    ...
    bool shouldUtilAutoMigrateForeignKeys(const std::string &param)
    {
        return shouldAutoMigrateForeignKeys() ||
               Common::isTruthyEnv(std::getenv(param.c_str()));
    }

    bool shouldAutoMigrateUsersForeignKeys()
    {
        return shouldUtilAutoMigrateForeignKeys("DB_AUTO_MIGRATE_USERS_FOREIGN_KEYS");
    }

    bool shouldAutoMigratePhonesForeignKeys()
    {
        return shouldUtilAutoMigrateForeignKeys("DB_AUTO_MIGRATE_PHONES_FOREIGN_KEYS");
    }

    bool shouldAutoMigrateOnlineDoctorsForeignKeys()
    {
        return shouldUtilAutoMigrateForeignKeys("DB_AUTO_MIGRATE_ONLINEDOCTORS_FOREIGN_KEYS");
    }

    bool shouldAutoMigrateReservationsForeignKeys()
    {
        return shouldUtilAutoMigrateForeignKeys("DB_AUTO_MIGRATE_RESERVATIONS_FOREIGN_KEYS");
    }

    bool shouldAutoMigratePetsForeignKeys()
    {
        return shouldUtilAutoMigrateForeignKeys("DB_AUTO_MIGRATE_PETS_FOREIGN_KEYS");
    }

    bool shouldAutoMigrateOrdersForeignKeys()
    {
        return shouldUtilAutoMigrateForeignKeys("DB_AUTO_MIGRATE_ORDERS_FOREIGN_KEYS");
    }

    bool shouldAutoMigrateOrderMedicinesForeignKeys()
    {
        return shouldUtilAutoMigrateForeignKeys("DB_AUTO_MIGRATE_ORDERMEDICINES_FOREIGN_KEYS");
    }

    bool shouldAutoMigrateWorkTimeRecordsForeignKeys()
    {
        return shouldUtilAutoMigrateForeignKeys("DB_AUTO_MIGRATE_WORKTIMERECORDS_FOREIGN_KEYS");
    }

    bool shouldAutoMigrateUserOperationsForeignKeys()
    {
        return shouldUtilAutoMigrateForeignKeys("DB_AUTO_MIGRATE_USER_OPERATIONS_FOREIGN_KEYS");
    }
    ...
    void migrateUsers(DatabaseManagerInterface &database_manager)
    {
        migrateForeignKeyIfEnabled(
            database_manager,
            {"users", "fk_users_position", "position_id", "positions", "id", "RESTRICT"},
    ...
    // 为 reservations 表补齐字段、索引和外键迁移逻辑。
    void migrateReservations(DatabaseManagerInterface &database_manager)
    {
        ...
        migrateForeignKeyIfEnabled(
            ...
            shouldUtilAutoMigrateForeignKeys("DB_AUTO_MIGRATE_RESERVATIONS_FOREIGN_KEYS"),
    ...
    void migratePets(DatabaseManagerInterface &database_manager)
    {
        migrateForeignKeyIfEnabled(
            ...
            shouldUtilAutoMigrateForeignKeys("DB_AUTO_MIGRATE_PETS_FOREIGN_KEYS"),
    ...
    void migrateOrderMedicines(DatabaseManagerInterface &database_manager)
    {
        migrateForeignKeyIfEnabled(
            ...
            shouldUtilAutoMigrateForeignKeys("DB_AUTO_MIGRATE_ORDERMEDICINES_FOREIGN_KEYS"),
    ...
    void migrateWorkTimeRecords(DatabaseManagerInterface &database_manager)
    {
        migrateForeignKeyIfEnabled(
            ...
            shouldUtilAutoMigrateForeignKeys("DB_AUTO_MIGRATE_WORKTIMERECORDS_FOREIGN_KEYS"),
            "DB_AUTO_MIGRATE_WORKTIMERECORDS_FOREIGN_KEYS");
    }

    // 为 user_operations 表补齐其外键迁移逻辑。
    void migrateUserOperations(DatabaseManagerInterface &database_manager)
    {
        migrateForeignKeyIfEnabled(
            ...
            shouldUtilAutoMigrateForeignKeys("DB_AUTO_MIGRATE_USER_OPERATIONS_FOREIGN_KEYS"),
