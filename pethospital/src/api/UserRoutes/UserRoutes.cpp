...
// 单例模式
UserRoutes &UserRoutes::instance()
{
    static UserRoutes instance;
    return instance;
}
void UserRoutes::UserRoutes::setupUserRoutes(crow::SimpleApp &app)
{
    // 添加标志防止重复设置路由
    ...
    CROW_ROUTE(app, "/api/user/form")
        .methods(crow::HTTPMethod::Delete, crow::HTTPMethod::Options)([](const crow::request &req, crow::response &res)
                                                                      {
        // 处理OPTIONS预检请求
        initializeOPTIONS(req, res);
